...
`,
  ...
  {
    "id": 4,
    "title": "Token bilan so'rov (Authorization)",
    "instruction": "getWithAuth(url, token) funksiyasini yozing: fetch ga headers ichida 'Authorization': 'Bearer <token>' bilan so'rov yuborsin va javobning JSON ini qaytarsin. Bu real loyihalarda autentifikatsiyaning eng keng tarqalgan usuli.",
    "startingCode": "async function getWithAuth(url, token) {\n  // Authorization: Bearer token headerini qo'shing\n}\n",
    "hint": "const res = await fetch(url, { headers: { Authorization: 'Bearer ' + token } }); return await res.json();",
    "test": "if (!code.includes('Authorization')) return 'Authorization headeri ishlatilmadi';\nlet captured = null;\nconst mockFetch = async (u, opt) => { captured = opt; return { json: async () => ({ me: true }) }; };\nconst sandbox = new Function('fetch', code + '; return getWithAuth;');\nconst fn = sandbox(mockFetch);\nconst out = await fn('/me', 'abc123');\nconst h = (captured && captured.headers) ? captured.headers : {};\nconst auth = h.Authorization || h.authorization;\nif (auth !== 'Bearer abc123') return 'Authorization: Bearer abc123 kutilgan edi, keldi: ' + auth;\nif (!out || out.me !== true) return 'JSON natija qaytarilmadi';\nreturn null;"
  },
  {
    "id": 5,
    "title": "So'rovni vaqt chegarasi bilan to'xtatish (Timeout)",
    "instruction": "fetchWithTimeout(url, ms) funksiyasini yozing: AbortController yaratsin, signal ni fetch ga uzatsin va ms dan keyin controller.abort() chaqirsin. Abort bo'lganda xato tashqariga otishi kerak (mock fetch signal ga 'abort' hodisasi bo'yicha reject bo'ladi).",
    "startingCode": "async function fetchWithTimeout(url, ms) {\n  // AbortController yarating, signal ni fetch ga bering\n  // setTimeout bilan ms dan keyin abort() chaqiring\n}\n",
    "hint": "const controller = new AbortController(); const t = setTimeout(() => controller.abort(), ms); try { return await fetch(url, { signal: controller.signal }); } finally { clearTimeout(t); }",
    "test": "if (!code.includes('AbortController')) return 'AbortController ishlatilmadi';\nlet captured = null;\nconst mockFetch = (u, opt) => {\n  captured = opt;\n  return new Promise((resolve, reject) => {\n    if (opt && opt.signal) opt.signal.addEventListener('abort', () => reject(new Error('AbortError')));\n    setTimeout(resolve, 300);\n  });\n};\nconst sandbox = new Function('fetch', code + '; return fetchWithTimeout;');\nconst fn = sandbox(mockFetch);\nreturn fn('/slow', 50).then(() => 'Abort ishlamadi — 50ms da bekor qilinishi kerak edi').catch(() => {\n  if (captured && captured.signal && captured.signal.aborted) return null;\n  return 'signal fetch ga uzatilmadi yoki abort() chaqirilmadi';\n});"
  },
  {
    "id": 6,
    "title": "HTTP statuslarni tasniflash",
    "instruction": "classify(res) funksiyasini yozing: u status xossali obyekt qabul qiladi. 2xx bo'lsa 'ok', 400-499 bo'lsa 'client', 500-599 bo'lsa 'server' qaytarsin. Math.floor(res.status / 100) yoki res.ok dan foydalanishingiz mumkin.",
    "startingCode": "function classify(res) {\n  // 'ok', 'client' yoki 'server' qaytaring\n}\n",
    "hint": "const group = Math.floor(res.status / 100); if (group === 2) return 'ok'; if (group === 4) return 'client'; if (group === 5) return 'server';",
    "test": "const sandbox = new Function(code + '; return classify;');\nconst fn = sandbox();\nif (fn({ status: 200, ok: true }) !== 'ok') return '200 ok bo\\'lishi kerak';\nif (fn({ status: 201, ok: true }) !== 'ok') return '201 ok bo\\'lishi kerak';\nif (fn({ status: 404, ok: false }) !== 'client') return '404 client bo\\'lishi kerak';\nif (fn({ status: 500, ok: false }) !== 'server') return '500 server bo\\'lishi kerak';\nreturn null;"
  }
],
