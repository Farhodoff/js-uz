...
## 5.5. 🧩 Error-First Callback (Node.js uslubi)

Node.js va eski kutubxonalarda callback yozishning rasmiy qoidasi bor: callback ning BIRINCHI argumenti har doim xato (error), IKKINCHI argumenti esa natija (data).

Muvaffaqiyatli holat: birinchi argument null, natija ikkinchida:

    callback(null, data);

Xatolik yuz berganda: birinchi argumentda xato, natija yo'q:

    callback(new Error('Fayl topilmadi'));
...
Foydalanish namunasi:
    ...
    fs.readFile('fayl.txt', (err, data) => {
      if (err) {
        ...
        return; // xato bo'lsa, davom etmaslik muhim!
      }
      console.log('Natija:', data);
    });

Bu qoidani unutsangiz, callback funksiyalaringiz xatolarni jimgina yo'qotib yuboradi.
...
`,
  ...
  {
    "id": 4,
    "title": "Error-First Callback (Node.js uslubi)",
    "instruction": "readFileSim(success, callback) funksiyasini yozing. U fayl o'qishni simulyatsiya qiladi: success true bo'lsa callback(null, 'malumot') chaqirsin (xato yo'q, natija bor), false bo'lsa callback('Fayl topilmadi') chaqirsin (xato BIRINCHI argumentda!).",
    "startingCode": "function readFileSim(success, callback) {\n  // Error-first qoidasi: xato — 1-argument, natija — 2-argument\n}\n",
    "hint": "if (success) { callback(null, 'malumot'); } else { callback('Fayl topilmadi'); }",
    "test": "const sandbox = new Function(code + '; return readFileSim;');\nconst fn = sandbox();\nlet err = 'INIT', data = 'INIT';\nfn(true, (e, d) => { err = e; data = d; });\nif (err !== null || data !== 'malumot') return 'Muvaffaqiyat holatida callback(null, data) chaqirilishi kerak';\nerr = null; data = null;\nfn(false, (e, d) => { err = e; data = d; });\nif (!err) return 'Xato holatida callback birinchi argumentda error olishi kerak';\nreturn null;"
  },
  {
    "id": 5,
    "title": "Parallel natijalarni yigish",
    "instruction": "runParallel(fn1, fn2, done) funksiyasini yozing: ikkala asinxron funksiyani (ular Promise qaytaradi) BIR VAQTDA ishga tushiring va ikkalasi tugagach done([natija1, natija2]) ni chaqiring. Ketma-ket kutib qolsangiz — bu parallel emas!",
    "startingCode": "function runParallel(fn1, fn2, done) {\n  // Ikkalasini parallel ishga tushiring, natijalarni massiv qilib done ga bering\n}\n",
    "hint": "Promise.all([fn1(), fn2()]).then(results => done(results));",
    "test": "const sandbox = new Function(code + '; return runParallel;');\nconst fn = sandbox();\nconst start = Date.now();\nreturn new Promise(resolve => {\n  fn(\n    () => new Promise(r => setTimeout(() => r('a'), 40)),\n    () => new Promise(r => setTimeout(() => r('b'), 40)),\n    (res) => {\n      if (!Array.isArray(res) || res[0] !== 'a' || res[1] !== 'b') { resolve('Natijalar noto\\'g\\'ri yig\\'ildi'); return; }\n      const elapsed = Date.now() - start;\n      if (elapsed > 70) { resolve('Ketma-ket bajarildi (' + elapsed + 'ms) — parallel kerak (~40ms)'); return; }\n      resolve(null);\n    }\n  );\n});"
  },
  {
    "id": 6,
    "title": "Bir marta ishlaydigan callback (once)",
    "instruction": "once(fn) funksiyasini yozing: u yangi funksiya qaytarsin. Qaytarilgan funksiya birinchi chaqiruvda fn ni ishga tushirsin, takroriy chaqiruvlarda esa CHAQIRMASIN. Real asinxron kodda callback ikki marta chaqirilishi jiddiy bug hisoblanadi!",
    "startingCode": "function once(fn) {\n  // Qaytarilgan funksiya fn ni faqat 1 marta chaqirsin (closure ishlating)\n  return function (...args) {\n\n  };\n}\n",
    "hint": "let called = false; flag ishlating: agar chaqirilmagan bo'lsa, called = true qilib fn(...args) chaqiring.",
    "test": "const sandbox = new Function(code + '; return once;');\nconst fn = sandbox();\nlet count = 0;\nconst inc = fn(() => count++);\ninc(); inc(); inc();\nif (count !== 1) return 'fn faqat 1 marta chaqirilishi kerak, hozir: ' + count + ' marta';\nreturn null;"
  }
],
