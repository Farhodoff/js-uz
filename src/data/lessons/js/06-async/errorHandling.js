...
`,
  ...
  {
    "id": 4,
    "title": "Custom Error klassi (ValidationError)",
    "instruction": "validatePassword(pw) funksiyasini yozing: parol 8 belgidan qisqa bo'lsa, ValidationError nomli maxsus xato otsin (Error dan extends qiling, this.name = 'ValidationError' qiling). To'g'ri parol bo'lsa true qaytarsin. Test maxsus xato nomini tekshiradi!",
    "startingCode": "class ValidationError extends Error {\n  constructor(message) {\n    super(message);\n    this.name = 'ValidationError';\n  }\n}\n\nfunction validatePassword(pw) {\n  // 8 belgidan qisqa bo'lsa throw qiling, aks holda true qaytaring\n}\n",
    "hint": "if (pw.length < 8) throw new ValidationError('Parol juda qisqa'); return true;",
    "test": "const sandbox = new Function(code + '; return validatePassword;');\nconst fn = sandbox();\ntry {\n  fn('qisqa');\n  return 'Qisqa parolda xato otilmadi';\n} catch (e) {\n  if (e.name !== 'ValidationError') return 'Xato turi ValidationError bolishi kerak, keldi: ' + e.name;\n  if (fn('uzunparol123') !== true) return 'Togri parol true qaytarmadi';\n  return null;\n}"
  },
  {
    "id": 5,
    "title": "Xato turlari boyicha shartli boshqarish",
    "instruction": "handleSafely(fn) funksiyasini yozing: fn() ni chaqiring. TypeError otilsa 'Tur xatosi' qaytaring, ReferenceError otilsa 'Havola xatosi', boshqa xatolarda 'Noma\\'lum xato'. Xato otmasa fn ning natijasini o'zini qaytaring.",
    "startingCode": "function handleSafely(fn) {\n  // try-catch bilan fn() chaqiring, xato turiga qarab har xil qiymat qaytaring\n}\n",
    "hint": "try { return fn(); } catch (e) { if (e instanceof TypeError) return 'Tur xatosi'; if (e instanceof ReferenceError) return 'Havola xatosi'; return 'Noma\\'lum xato'; }",
    "test": "const sandbox = new Function(code + '; return handleSafely;');\nconst fn = sandbox();\nif (fn(() => { throw new TypeError('t'); }) !== 'Tur xatosi') return 'TypeError togri tasniflanmadi';\nif (fn(() => { throw new ReferenceError('r'); }) !== 'Havola xatosi') return 'ReferenceError togri tasniflanmadi';\nif (fn(() => { throw new Error('e'); }) !== 'Noma\\'lum xato') return 'Boshqa xato togri tasniflanmadi';\nif (fn(() => 42) !== 42) return 'Xato bolmaganda fn natijasi qaytmadi';\nreturn null;"
  },
  {
    "id": 6,
    "title": "Asinxron xatolarni ushlash (Result pattern)",
