...
import { curriculum, SECTIONS, NEXT_STEPS_START } from '../../data/curriculum';
    ...
    <nav className={`sidebar ${sidebarOpen ? 'open' : ''}`} role="navigation" aria-label="Darslar navigatsiyasi">
      ...
      <div className="sidebar-content">
        ...
        {currentSections.filter(key => key !== 'challenges').map(key => {
          ...
          const isNextSteps = key === NEXT_STEPS_START;

          return (
            <div key={key}>
              {isNextSteps && (
                <div style={{ borderTop: '1px dashed #334155', margin: '14px 12px 2px', paddingTop: '12px', fontSize: '11px', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '1.5px' }}>
                  🚀 Keyingi qadamlar
                </div>
              )}
              <button
              ...
              {isActive && s.lessons.map(l => (
                ...
                >
                  ...
                  {l.number && <span style={{ color: '#64748b', fontSize: '11px', minWidth: '24px' }}>{l.number}</span>}
                  {l.title}
                </button>
              ))}
            </div>
